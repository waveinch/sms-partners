...
trait Tables {
  ...
  case class SmsLogsRow(id: Int, messageSid: String, accountSid: String, messagingServiceSid: String, fromPhone: String, toPhone: String, body: String)
  /** GetResult implicit for fetching SmsLogsRow objects using plain SQL queries */
  implicit def GetResultSmsLogsRow(implicit e0: GR[Int], e1: GR[String]): GR[SmsLogsRow] = GR{
    prs => import prs._
    SmsLogsRow.tupled((<<[Int], <<[String], <<[String], <<[String], <<[String], <<[String], <<[String]))
  }
  ...
  class SmsLogs(_tableTag: Tag) extends Table[SmsLogsRow](_tableTag, "sms_logs") {
    def * = (id, messageSid, accountSid, messagingServiceSid, fromPhone, toPhone, body) <> (SmsLogsRow.tupled, SmsLogsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(messageSid), Rep.Some(accountSid), Rep.Some(messagingServiceSid), Rep.Some(fromPhone), Rep.Some(toPhone), Rep.Some(body)).shaped.<>({r=>import r._; _1.map(_=> SmsLogsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
